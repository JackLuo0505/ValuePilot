...
Create a scenario within a household where 2-4 agents interact with each other or one singal agents interact with the environment around him. Ensure that the agents' actions will embody {values[0]}, while DO NOT embody other basic values mentioned before.
...
AFTER YOU GENERATE YOUR TEXT, YOU MUST CHECK OUT IF ANY SENTENCES INCLUDE {values[0]} OR ALL OF THEIR SYNONYMS OR SAME-ROOTED WORDS. IF SO, DELETE THOSE SENTENCES.
...
The agent A's actions should embody out {values[0]}.
...
AFTER YOU GENERATE ACTIONS, YOU MUST CHECK OUT IF ANY ACTION DESCRIPTIONS INCLUDE {values[0]} OR ALL OF THEIR SYNONYMS OR SAME-ROOTED WORDS. IF SO, DELETE THOSE ACTIONS AND CREATE NEW ONES.
...
2. The scenario DO NOT contain other values besides {values[0]}.
...
5. Action Scoring
If the scenario is suitable for training, assign scores to each action on a scale of -1.0 to 1.0 on {values[0]}.
Both {values[0]} need to be scored, and they should not be zero, as the generated scenarios are closely related to {values[0]}.
...
!!!REMEMBER!!!
RULE: {values[0]} need to be SCORED for each action, and they should not be zero!!!
...
def generate_aspect_scores_array(value1, value1_score):
...
def generate_list(data_string, value1):
    # Define field names
    ...
    # Add value1 to the "values" key in the main dictionary
    value_list = [value1]
...
def generate_scenarios_for_pairs(pair_start, pair_end, pair_range=80, mode='train'):
    ...
    value_pairs = list(itertools.combinations(value_dimensions, 1))
    # Create a scenarios folder if it doesn't exist
    scenarios_folder = './dataset_gpt4/value_1/{}_data/'.format(mode)
    ...
    for pair_index, pair in enumerate(value_pairs[pair_start:pair_end]):
        ...
        while idx < pair_range:
            ...
            descriptions = [desciption_matrix[values_generation[0]]]
            ...
            if summary:
                scenario_list = generate_list(summary, values_generation[0])
                if scenario_list:
