...
Create a scenario within a household where 2-4 agents interact with each other or one singal agents interact with the environment around him. Ensure that the agents' actions will embody {values[0]} and {values[1]} and {values[2]} and {values[3]} and {values[4]} and {values[5]}, while DO NOT embody other basic values mentioned before.
{values[0]} means {desciptions[0]}.
{values[1]} means {desciptions[1]}.
{values[2]} means {desciptions[2]}.
{values[3]} means {desciptions[3]}.
{values[4]} means {desciptions[4]}.
{values[5]} means {desciptions[5]}.
...
AFTER YOU GENERATE YOUR TEXT, YOU MUST CHECK OUT IF ANY SENTENCES INCLUDE {values[0]} OR {values[1]} OR {values[2]} OR {values[3]} OR {values[4]} OR {values[5]} ALL OF THEIR SYNONYMS OR SAME-ROOTED WORDS. IF SO, DELETE THOSE SENTENCES.
...
The agent A's actions should embody out {values[0]} and {values[1]} and {values[2]} and {values[3]} and {values[4]} and {values[5]}.
...
AFTER YOU GENERATE ACTIONS, YOU MUST CHECK OUT IF ANY ACTION DESCRIPTIONS INCLUDE {values[0]} OR {values[1]} OR {values[2]} OR {values[3]} OR {values[4]} OR {values[5]} OR ALL OF THEIR SYNONYMS OR SAME-ROOTED WORDS. IF SO, DELETE THOSE ACTIONS AND CREATE NEW ONES.
...
2. The scenario DO NOT contain other values besides {values[0]} and {values[1]} and {values[2]} and {values[3]} and {values[4]} and {values[5]}.
...
5. Action Scoring
If the scenario is suitable for training, assign scores to each action on a scale of -1.0 to 1.0 on {values[0]}, {values[1]} and {values[2]} and {values[3]} and {values[4]} and {values[5]}.
Both {values[0]} and {values[1]}and {values[2]} and {values[3]} and {values[4]} and {values[5]} need to be scored, and they should not be zero, as the generated scenarios are closely related to {values[0]} and {values[1]} and {values[2]} and {values[3]} and {values[4]} and {values[5]}.
...
!!!REMEMBER!!!
RULE: ALL {values[0]} and {values[1]} and {values[2]} and {values[3]} and {values[4]} and {values[5]} need to be SCORED for each action, and they should not be zero!!!
REMEMBER THAT THERE SHOUNL'D BE ACTIONS WITH ONLY ONE OR TWO OR THREE VALUES SCORED AND OTHERS SCORED ZERO.
IF YOU GENERATED ANY ACTIONS WITHOUT 3 UNZERO SCORED VALUES , YOU MUST DELETE IT. 
...
-{values[3]}: [{values[3]} of action 1 scoring]
-{values[4]}: [{values[4]} of action 1 scoring]
-{values[5]}: [{values[5]} of action 1 scoring]
...
-{values[3]}: [{values[3]} of action 2 scoring]
-{values[4]}: [{values[4]} of action 2 scoring]
-{values[5]}: [{values[5]} of action 2 scoring]
...
...
def generate_aspect_scores_array(value1, value2, value3, value4, value5, value6, value1_score, value2_score, value3_score, value4_score, value5_score, value6_score):
    ...
    value4_lower = value4.lower()
    value4_index = aspects.index(value4_lower) if value4_lower in aspects else None

    value5_lower = value5.lower()
    value5_index = aspects.index(value5_lower) if value5_lower in aspects else None

    value6_lower = value6.lower()
    value6_index = aspects.index(value6_lower) if value6_lower in aspects else None
    ...
    if value4_index is not None and isinstance(value4_score, (float, int, str)):
        scores[value4_index] = float(value4_score)
    
    if value5_index is not None and isinstance(value5_score, (float, int, str)):
        scores[value5_index] = float(value5_score)

    if value6_index is not None and isinstance(value6_score, (float, int, str)):
        scores[value6_index] = float(value6_score)

    return scores

def generate_list(data_string, value1, value2, value3, value4, value5, value6):
    # Define field names
    ...
    value_list = [value1, value2,value3,value4,value5,value6]
    ...
    for index, line in enumerate(lines[1:], start=1):
        ...
        if line.startswith("Action:") or line.lower().replace(" ", "").startswith(f"-{value1}:") or line.lower().replace(" ", "").startswith(f"-{value2}:") or line.lower().replace(" ", "").startswith(f"-{value3}:") or line.lower().replace(" ", "").startswith(f"-{value4}:") or line.lower().replace(" ", "").startswith(f"-{value5}:") or line.lower().replace(" ", "").startswith(f"-{value6}:"):
            # If the action_description is not empty, it means we have completed the previous action
            
            if action_description and value1_scoring_part != "" and value2_scoring_part != "" and value3_scoring_part != "" and value4_scoring_part != "" and value5_scoring_part != "" and value6_scoring_part != "":
                
                # Create a dictionary for the completed action and add it to the action list
                dict_action = {
                    fields_action[0]: action_description,
                    fields_action[1]: generate_aspect_scores_array(value1, value2, value3, value4 , value5, value6, value1_scoring_part, value2_scoring_part, value3_scoring_part, value4_scoring_part, value5_scoring_part, value6_scoring_part)
                }
            ...
            elif line.lower().replace(" ", "").startswith(f"-{value4}:"):
                value4_scoring_part = extract_scores(line.lower(), value4)

            elif line.lower().replace(" ", "").startswith(f"-{value5}:"):
                value5_scoring_part = extract_scores(line.lower(), value5)

            elif line.lower().replace(" ", "").startswith(f"-{value6}:"):
                value6_scoring_part = extract_scores(line.lower(), value6)
...
def generate_scenarios_for_pairs(pair_start, pair_end, pair_range=80, mode='train'):
    ...
    value_pairs = list(itertools.combinations(value_dimensions, 6))
    # Create a scenarios folder if it doesn't exist
    scenarios_folder = './dataset_gpt4/value_6/{}_data/'.format(mode)
    ...
    for pair_index, pair in enumerate(value_pairs[pair_start:pair_end]):
        ...
        json_file_name = f"{pair_index}_{pair[0]}_{pair[1]}_{pair[2]}_{pair[3]}_{pair[4]}_{pair[5]}_{mode}.json"
        json_file_path = os.path.join(scenarios_folder, json_file_name)

        # Generate a unique filename for scenario_text
        text_file_name = f"{pair_index}_{pair[0]}_{pair[1]}_{pair[2]}_{pair[3]}_{pair[4]}_{pair[5]}_{mode}_text.txt"
        ...
        while idx < pair_range:
            ...
            descriptions = [desciption_matrix[values_generation[0]], desciption_matrix[values_generation[1]],desciption_matrix[values_generation[2]], desciption_matrix[values_generation[3]], desciption_matrix[values_generation[4]], desciption_matrix[values_generation[5]]]
            ...
            if summary:
                scenario_list = generate_list(summary, values_generation[0], values_generation[1],  values_generation[2], values_generation[3], values_generation[4], values_generation[5])
                if scenario_list:
